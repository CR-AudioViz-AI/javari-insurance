"use client";
// app/page.tsx — Javari Insurance · CR AudioViz AI · EIN 39-3646201 · May 2026
import { useState } from "react";
const T=[{"i": "\ud83d\udccb", "l": "Policy Summary", "d": "Plain-English policies", "h": "/policy"}, {"i": "\ud83d\udce7", "l": "Client Email", "d": "Renewal and coverage updates", "h": "/email"}, {"i": "\ud83d\udcdd", "l": "Claims Summary", "d": "Claims documentation", "h": "/claims"}, {"i": "\ud83d\udcf1", "l": "Agent Marketing", "d": "Insurance agent content", "h": "/marketing"}];
export default function P() {
  ...
  async function go() { if(!i.trim())return; setL(true);setO("");
    try { const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:i}],stream:false,systemOverride:"You are an insurance professional. Always recommend consulting a licensed agent."})});
  ...
  return (<div style={{minHeight:"100vh",background:"#040912",color:"#e2e8f0",fontFamily:"system-ui"}}>
    ...
    <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"48px 24px 40px",textAlign:"center"}}>
      <h1 style={{fontSize:"clamp(22px,4vw,42px)",fontWeight:900,color:"#fff",margin:"0 0 10px",lineHeight:1.05}}>AI Tools for<br/><span style={{color:"#00B4D8"}}>Insurance Professionals</span></h1>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:15,margin:0}}>Policy explanations, client comms, and insurance marketing.</p>
    </section>
